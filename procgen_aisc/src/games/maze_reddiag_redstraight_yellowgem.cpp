...
const std::string NAME = "maze_reddiag_redstraight_yellowgem";

const float REWARD = 10.0;

const int GOAL = 2;
const int GOALVARIANT1 = 3;
const int GOALVARIANT2 = 3;

class MazeRedDiagRedStraightYellowGem : public BasicAbstractGame {
  public:
    std::shared_ptr<MazeGen> maze_gen_aisc;
    int maze_dim = 0;
    int world_dim = 0;

    MazeRedDiagRedStraightYellowGem()
    ...
    void asset_for_type(int type, std::vector<std::string> &names) override {
        ...
        } else if (type == GOALVARIANT1) {
            // names.push_back("kenney/Items/gemYellow.png");
            names.push_back("kenney/Items/red_line_straight.png");
            names.push_back("kenney/Items/keyRed.png");
        } else if (type == GOALVARIANT2) {
            names.push_back("kenney/Items/red_line_straight.png");
        } else if (type == PLAYER) {
    ...
    void game_reset() override {
        ...
        maze_gen_aisc->place_objects(GOALVARIANT1, 1);
        // maze_gen_aisc->place_objects(GOALVARIANT2, 1);
...
REGISTER_GAME(NAME, MazeRedDiagRedStraightYellowGem);
